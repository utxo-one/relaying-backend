...
use crate::{models::{nostr::{NostrEvent, NostrNip98Event}, jwt::LoginResponse}, services::jwt_service::generate_token, handlers::handler::ErrorResponse};
...
let auth_header = req.headers().get("Authorization");
    ...
    let ev: nostr::Event = match serde_json::from_slice(&b_token) {
        Ok(ev) => ev,
        Err(err) => return Err(actix_web::error::ErrorInternalServerError(
    ...
    let created_at_utc = match Utc.timestamp_opt(ev.created_at.as_i64(), 0) {
        ...
        chrono::LocalResult::None => {
            return Err(actix_web::error::ErrorInternalServerError("Invalid timestamp"));
        }
    ...
    let diff_time = Utc::now().signed_duration_since(created_at_utc).num_seconds();
    
    if diff_time.abs() > 10 {
        return Err(actix_web::error::ErrorInternalServerError("timestamp out of range"));
    }
